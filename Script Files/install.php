...
if(isset($_POST['submit'])) {
    $xclone_db_host = $_POST['db_host'];
    $xclone_db_user = $_POST['db_user'];
    $xclone_db_pass = $_POST['db_pass'];
    $xclone_db_name = $_POST['db_name'];
    $filename = 'core/dbconnect.phtml';
    $content = '<?php' . PHP_EOL;
    $content .= '$xclone_db_host = \'' . $xclone_db_host . '\';' . PHP_EOL;
    $content .= '$xclone_db_user = \'' . $xclone_db_user . '\';' . PHP_EOL;
    $content .= '$xclone_db_pass = \'' . $xclone_db_pass . '\';' . PHP_EOL;
    $content .= '$xclone_db_name = \'' . $xclone_db_name . '\';' . PHP_EOL;
    ...
    $sql_file = 'xclone-sm.sql';
    $command = "mysql -u {$xclone_db_user} -p{$xclone_db_pass} -h {$xclone_db_host} {$xclone_db_name} < $sql_file";
  
    $mysql = new mysqli($xclone_db_host, $xclone_db_user, $xclone_db_pass, $xclone_db_name);
    ...
    if ($mysql->multi_query(file_get_contents($sql_file))) {
      ...
      Xclone has been successfully installed!<br><br> You will be redirected to the homepage in 10 seconds. <br><br> To log in as an administrator: use "admin" - "admin" and dont forget to delete the "install.php" file in the main directory.
      </div>';
      $url = '/welcome/login';
      $delay = 3;
      header("Refresh: $delay; url=$url");
...
<html lang="en">
  <head>
    ...
    <title>Automatic Installation | Xclone - PHP Twitter Social Networking Platform</title>
  </head>
  <body>
  ...
  <div class="col-md-6">
    <div class="xclone-install justify-content-center align-items-center">
        <div class="install-div text-center">
            ...
            <small>With automatic installation, you will be able to install your Xclone script in seconds!</small>
        </div>
        <div class="install-form-group">
            <form method="post">
                ...
                <button type="submit" name="submit" class="btn btn-success">Install Xclone</button>
            </form>
        </div>
...
<div class="col-md-6">
  <div class="php-xclone-modules" style="padding: 30px; background: #1da1f2; min-height: 100vh; color: #fff;">
    <div class="container mt-5">
      <div class="row justify-content-center">
        ...
        <div class="modules-descpr" style="position: relative; margin-bottom: 40px;">
          ...
          line-height: 25px;
          "
          >Please make sure your server fulfills the requirements to run Xclone.</h5>
        </div>
