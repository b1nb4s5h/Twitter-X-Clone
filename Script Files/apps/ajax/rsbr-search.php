...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($search)) {
      ...
      $sql_users = "SELECT * FROM xclone_users WHERE account_status = 0 AND (fullname LIKE ? OR username LIKE ?) AND id <> ?";
      ...
      $sql_hashtags = "SELECT * FROM xclone_hashtag WHERE htag LIKE ?";
      ...
      if ($result_users->num_rows > 0 || $result_hashtags->num_rows > 0) {
        ...
        while ($row = $result_users->fetch_assoc()) {
          ...
          echo '<div class="right-sidebar-live-search-container" data-xclone-url="' . xclone_link("@" . $row["username"]) . '">';
          echo '<div class="livesearch-user-info">';
          echo '<a href="' . xclone_link("@" . $row["username"]) . '">';
          echo '<img src="' . xclone_link($row["avatar"]) . '">';
          echo '</a>';
          echo '<h5>' . $row["username"];
          
          if ($row["verified"] == "1") {
              echo xclone_userBlue();
          }
          
          echo '</h5>';
          echo '<p>';
          echo '<span>' . $row["fullname"] . ' • </span>';
          echo '<small>' . $row["followers"] . ' ' . xclone_translate("my_followers") . '</small>';
          echo '</p>';
          echo '</div>';
          echo '</div>';
        }
          ...
          while ($row = $result_hashtags->fetch_assoc()) {
              echo '<div class="livesearch-userbox" data-xclone-url="' . xclone_link("explore?q=" . $row["htag"]) . '">';
